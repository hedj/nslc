  ...
  while(True):
    ...
    if( len(inb) >= 2):
      if inb[-1] == 10 and inb[-2] != 9:
        if(is_valid_frame(inb)):
          print("Found valid frame %s"%unframe(inb))
        else:
          print("Found invalid frame %s"%unframe(inb))
        inb = b''
